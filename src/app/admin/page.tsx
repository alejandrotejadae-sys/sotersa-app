    ...
    <main className="min-h-dvh bg-[#020b18] text-white">
      <div className="mx-auto min-h-dvh w-full max-w-[540px] border-x border-white/[0.04] bg-[radial-gradient(circle_at_50%_-5%,rgba(0,128,255,0.14),transparent_34%),linear-gradient(180deg,#020b18,#031226_55%,#020b18)] shadow-2xl shadow-black/40">
        ...
        <div className="space-y-4 px-4 pb-28">
          ...
          <section className="rounded-2xl border border-[#27425e] bg-[#07172a]/95 p-4"><h2 className="text-lg font-semibold">Acciones rápidas</h2><div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4"><Accion icono={<IconoCiclo className="h-7 w-7" />} texto="Rondas"/><Accion icono={<IconoLista className="h-7 w-7" />} texto="Incidentes"/><Accion icono={<Camara className="h-7 w-7" />} texto="Cámaras"/><Accion icono={<IconoAlerta className="h-7 w-7" />} texto="Alertas"/></div></section>
...
function Accion({ icono, texto }: { icono: React.ReactNode; texto: string }) { return <button type="button" className="flex min-h-24 flex-col items-center justify-center gap-2 rounded-xl border border-[#27425e] bg-[#061426] p-3 text-sm text-slate-300"><span className="text-[#0788ff]">{icono}</span>{texto}</button>; }
function Nav({ icono, texto, activo = false }: { icono: React.ReactNode; texto: string; activo?: boolean }) { return <button type="button" className={`flex min-h-14 flex-col items-center justify-center gap-1 text-[0.68rem] ${activo ? "text-[#0788ff]" : "text-slate-400"}`}>{icono}<span>{texto}</span></button>; }

...
export default async function PaginaAdmin() {
  ...
  const [clientesR, guardiasR, custodiasR, supervisoresR, usuariosR, alertasR, puestosR, turnosR, rondasR, aperturasR, novedades24R] = await Promise.all([
    ...
    supabase.from("novedades").select("id,tipo,severidad,hora_captura,puestos(codigo,nombre,empresas_cliente(nombre)),guardias(nombre)", { count: "exact" }).eq("estado", "registrada").order("hora_captura", { ascending: false }).limit(100),
    ...
    supabase.from("rondas").select("id,hora_captura", { count: "exact" }).gte("hora_captura", desde24h),
    supabase.from("aperturas_turno").select("id,hora_captura", { count: "exact" }).gte("hora_captura", desde24h),
    supabase.from("novedades").select("id,hora_captura,severidad", { count: "exact" }).gte("hora_captura", desde24h),
  ]);
  ...
  const prioridades = {
    emergencia: alertas.filter((a) => a.severidad === "emergencia").length,
    novedad: alertas.filter((a) => a.severidad === "novedad").length,
    informativa: alertas.filter((a) => a.severidad === "informativa").length,
  };
  const actividad = construirActividad24h([
    ...(rondasR.data ?? []).map((r) => r.hora_captura),
    ...(aperturasR.data ?? []).map((a) => a.hora_captura),
    ...(novedades24R.data ?? []).map((n) => n.hora_captura),
  ], new Date(ahora));
      ...
      <div className="mx-auto w-full max-w-[1560px] px-4 pb-28 pt-5 sm:px-6 lg:px-8 lg:pb-10 lg:pt-8">
        ...
        <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1.55fr)_minmax(20rem,.75fr)]">
          ...
          <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-1">
            ...
            <section className="sot-card overflow-hidden">
              <div className="flex items-center justify-between border-b border-[#173651] px-5 py-4"><h2 className="flex items-center gap-2 font-semibold"><IconoAlerta className="h-5 w-5 text-azul-300" /> Alertas y prioridades</h2><Link href="/operacion/novedades" className="text-xs font-semibold text-azul-300">Ver todas</Link></div>
              <div className="grid grid-cols-3 gap-2 border-b border-[#173651] p-4">
                <Prioridad etiqueta="Críticas" valor={prioridades.emergencia} tono="rojo" />
                <Prioridad etiqueta="Atención" valor={prioridades.novedad} tono="ambar" />
                <Prioridad etiqueta="Informativas" valor={prioridades.informativa} tono="azul" />
              </div>
              {alertas.length === 0 ? <div className="px-5 py-8 text-center"><span className="mx-auto grid h-11 w-11 place-items-center rounded-full bg-emerald-500/10 text-emerald-300"><IconoEscudoOk className="h-6 w-6" /></span><p className="mt-3 text-sm font-medium text-slate-200">Operación sin alertas pendientes</p><p className="mt-1 text-xs text-slate-500">Las nuevas novedades aparecerán aquí.</p></div> : <div className="divide-y divide-[#173651]">{alertas.slice(0, 4).map((alerta) => <Alerta key={alerta.id} alerta={alerta} />)}</div>}
            </section>
          </div>
        </div>

        <section className="sot-card mt-5 overflow-hidden">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#173651] px-5 py-4">
            <div><h2 className="font-semibold text-white">Actividad de las últimas 24 horas</h2><p className="mt-1 text-xs text-slate-500">Aperturas de turno, rondas y novedades registradas</p></div>
            <div className="flex flex-wrap gap-3 text-xs text-slate-400"><span><strong className="text-emerald-300">{aperturasR.count ?? 0}</strong> aperturas</span><span><strong className="text-azul-300">{rondasR.count ?? 0}</strong> rondas</span><span><strong className="text-amber-200">{novedades24R.count ?? 0}</strong> novedades</span></div>
          </div>
          <div className="px-5 pb-5 pt-6">
            <div className="grid h-40 grid-cols-6 items-end gap-2 sm:gap-4" aria-label={`${actividad.reduce((s, p) => s + p.valor, 0)} registros operativos en 24 horas`}>
              {actividad.map((punto) => <div key={punto.etiqueta} className="flex h-full flex-col justify-end gap-2 text-center"><span className="text-xs font-semibold text-slate-300">{punto.valor}</span><div className="mx-auto w-full max-w-16 rounded-t-lg bg-gradient-to-t from-azul-700 to-cyan-300" style={{ height: `${punto.altura}%`, minHeight: punto.valor ? "0.75rem" : "0.25rem", opacity: punto.valor ? 1 : .2 }} /><span className="text-[0.65rem] text-slate-500">{punto.etiqueta}</span></div>)}
            </div>
          </div>
        </section>
...
function Prioridad({ etiqueta, valor, tono }: { etiqueta: string; valor: number; tono: "rojo" | "ambar" | "azul" }) { const clase = { rojo: "border-red-500/25 bg-red-500/10 text-red-200", ambar: "border-amber-400/25 bg-amber-400/10 text-amber-200", azul: "border-azul-500/25 bg-azul-500/10 text-azul-200" }[tono]; return <div className={`rounded-xl border px-2 py-2.5 text-center ${clase}`}><strong className="block text-xl">{valor}</strong><span className="text-[0.65rem]">{etiqueta}</span></div>; }
...
function construirActividad24h(fechas: string[], ahora: Date) {
  const intervalo = 4 * 60 * 60 * 1000;
  const inicio = ahora.getTime() - 24 * 60 * 60 * 1000;
  const valores = Array.from({ length: 6 }, () => 0);
  for (const fecha of fechas) {
    const indice = Math.floor((new Date(fecha).getTime() - inicio) / intervalo);
    if (indice >= 0 && indice < valores.length) valores[indice] += 1;
  }
  const maximo = Math.max(...valores, 1);
  return valores.map((valor, indice) => ({
    etiqueta: indice === 5 ? "Ahora" : `-${20 - indice * 4} h`,
    valor,
    altura: Math.max(3, Math.round((valor / maximo) * 100)),
  }));
}

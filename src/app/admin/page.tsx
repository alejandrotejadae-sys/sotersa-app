  ...
  return (
    <main className="min-h-dvh bg-[#020b18] text-white">
      <div className="mx-auto min-h-dvh w-full max-w-[1440px] border-x border-white/[0.04] bg-[radial-gradient(circle_at_50%_-5%,rgba(0,128,255,0.14),transparent_34%),linear-gradient(180deg,#020b18,#031226_55%,#020b18)] shadow-2xl shadow-black/40">
        <header className="flex items-center justify-between gap-5 px-5 pb-4 pt-[max(1rem,env(safe-area-inset-top))] lg:px-8"><Marca tamano="panel" /><nav className="hidden items-center gap-1 lg:flex"><EnlaceSuperior href="/admin" texto="Inicio" activo/><EnlaceSuperior href="/operacion/personal" texto="Personal"/><EnlaceSuperior href="/operacion/rondas" texto="Rondas"/><EnlaceSuperior href="/supervisor" texto="Supervisor"/><EnlaceSuperior href="/portal" texto="Clientes"/></nav><span className="flex items-center gap-2 rounded-full border border-emerald-500/25 bg-emerald-500/10 px-3 py-2 text-xs font-medium text-emerald-300"><Pulso /> En línea</span></header>
        ...
        <div className="grid grid-cols-1 gap-4 px-4 pb-28 lg:grid-cols-12 lg:px-8 lg:pb-10">
          ...
          <section className="rounded-2xl border border-[#27425e] bg-[#07172a]/95 p-4 lg:col-span-5"><h2 className="text-lg font-semibold">Acciones rápidas</h2><div className="mt-3 grid grid-cols-2 gap-3"><Accion href="/operacion/rondas" icono={<IconoCiclo className="h-7 w-7" />} texto="Rondas"/><Accion icono={<IconoLista className="h-7 w-7" />} texto="Incidentes"/><Accion icono={<Camara className="h-7 w-7" />} texto="Cámaras"/><Accion icono={<IconoAlerta className="h-7 w-7" />} texto="Alertas"/></div></section>

          <section className="overflow-hidden rounded-2xl border border-[#27425e] bg-[#07172a]/95 lg:col-span-12"><div className="flex items-center justify-between border-b border-[#20374e] px-4 py-3.5"><h2 className="text-lg font-semibold">Actividad reciente</h2><span className="text-sm text-[#0788ff]">Ver todo</span></div>{novedades.length === 0 ? <p className="px-4 py-8 text-center text-sm text-slate-400">Todavía no hay actividad registrada.</p> : <div className="grid divide-y divide-[#20374e] lg:grid-cols-2 lg:divide-y-0">{novedades.map((novedad) => <article key={novedad.id} className="grid grid-cols-[2.5rem_1fr_auto] items-center gap-3 border-[#20374e] px-4 py-3 lg:border-b lg:odd:border-r"><span className={`grid h-10 w-10 place-items-center rounded-full border ${novedad.severidad === "emergencia" ? "border-red-400 text-red-300" : "border-[#0788ff] text-[#0788ff]"}`}>{novedad.severidad === "emergencia" ? "!" : "✓"}</span><div className="min-w-0"><p className="truncate text-sm font-medium">{novedad.tipo}</p><p className="truncate text-xs text-slate-400">{uno(novedad.puestos)?.codigo ?? uno(novedad.guardias)?.nombre ?? "Central"}</p></div><time className="text-xs text-slate-400">{horaEcuador(novedad.hora_captura)}</time></article>)}</div>}</section>
        </div>

...
export function FormularioUsuario({ empresas, zonas, guardias }: { empresas: { id: string; nombre: string }[]; zonas: { id: string; nombre: string }[]; guardias: { id: string; nombre: string; cedula: string | null }[] }) {
  ...
  const [rol, setRol] = useState<"cliente" | "supervisor" | "guardia" | "admin" | "operativo">("cliente");
  ...
  return (
    <form action={accion} className="space-y-4">
      <Campo etiqueta="Tipo de usuario"><select name="rol" value={rol} onChange={(evento) => setRol(evento.target.value as typeof rol)} className={control}><option value="cliente">Cliente</option><option value="supervisor">Supervisor</option><option value="guardia">Agente de seguridad</option><option value="operativo">Operativo</option><option value="admin">Administrador</option></select></Campo>
      ...
      {rol === "supervisor" && <><Campo etiqueta="Zona (opcional, solo referencia)"><select name="zona_id" defaultValue="" className={control}><option value="">Sin zona</option>{zonas.map((zona) => <option key={zona.id} value={zona.id}>{zona.nombre}</option>)}</select></Campo><p className="rounded-xl border border-[#0788ff]/25 bg-[#0788ff]/8 px-3 py-3 text-xs leading-5 text-[#b9e6ff]">Los puestos que supervisa se asignan después, en su ficha de usuario.</p></>}

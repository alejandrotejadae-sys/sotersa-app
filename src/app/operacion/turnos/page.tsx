...
function FilaTurno({ turno }: { turno: TurnoVista }) { const guardia = uno(turno.guardias); const puesto = uno(turno.puestos); const apertura = turno.aperturas_turno?.[0]; const verificados = apertura ? Object.values((apertura.checklist ?? {}) as Record<string, boolean>).filter(Boolean).length : 0; return <article className="grid grid-cols-[2.7rem_1fr_auto] items-center gap-3 px-4 py-3.5"><span className={`grid h-11 w-11 place-items-center rounded-full border ${apertura ? "border-emerald-500/35 bg-emerald-500/10 text-emerald-300" : "border-amber-500/35 bg-amber-500/10 text-amber-300"}`}>{apertura ? "✓" : "○"}</span><div className="min-w-0"><p className="truncate font-medium">{guardia?.nombre ?? "Guardia"}</p><p className="truncate text-sm text-slate-400">{puesto ? `${puesto.codigo} · ${puesto.nombre}` : "Puesto"}</p><p className="mt-1 text-xs text-slate-500">{horaEcuador(turno.inicio_programado)}–{horaEcuador(turno.fin_programado)}{apertura ? ` · ${verificados}/4 equipos` : " · sin apertura"}</p></div><EstadoTurno abierto={Boolean(apertura)} estado={turno.estado} /></article>; }
function FilaProxima({ turno }: { turno: TurnoVista }) { const guardia = uno(turno.guardias); const puesto = uno(turno.puestos); return <article className="grid grid-cols-[2.5rem_1fr_auto] items-center gap-3 border-[#20374e] px-4 py-3.5 lg:border-b lg:odd:border-r"><span className="grid h-10 w-10 place-items-center rounded-full border border-[#27425e] bg-[#061426] text-[#49b6ff]"><IconoPersona className="h-5 w-5" /></span><div className="min-w-0"><p className="truncate font-medium">{guardia?.nombre ?? "Guardia"}</p><p className="truncate text-sm text-slate-400">{puesto?.codigo ?? "Puesto"} · {etiquetaTipo(turno.tipo)}</p></div><time className="text-right text-xs leading-5 text-slate-400">{fechaHoraEcuador(turno.inicio_programado)}</time></article>; }

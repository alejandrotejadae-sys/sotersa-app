...
import { FormularioCargaPersonal } from "./formulario-carga-personal";
...
export default async function PaginaPersonal() {
  ...
  const [guardiasR, puestosR, turnosR] = await Promise.all([
    supabase.from("guardias").select("id, nombre, cedula, credencial, telefono, activo, es_relevo, plaza:puesto_habitual_id(codigo, nombre, empresas_cliente(nombre))").order("nombre"),
  ...
  return (
    <main className="min-h-dvh bg-[#020b18] text-white">
      <div className="mx-auto min-h-dvh w-full max-w-[1280px] bg-[radial-gradient(circle_at_50%_-5%,rgba(0,128,255,0.14),transparent_34%),linear-gradient(180deg,#020b18,#031226_55%,#020b18)] px-4 pb-10 pt-[max(1rem,env(safe-area-inset-top))] lg:px-8">
        ...
        <section className="mt-5 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="flex items-center gap-2 text-base font-medium text-[#0788ff]"><IconoPersona className="h-6 w-6" /> Gestión operativa</p><h1 className="mt-2 text-3xl font-bold">Agentes de seguridad</h1><p className="mt-1 text-sm text-slate-400">Toca un agente para abrir su ficha: datos, plaza, acceso y actividad.</p></div>{perfil.rol === "admin" && <Link href="/operacion/turnos" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-[#0788ff]/15 px-4 text-sm font-semibold text-[#65c8ff]"><IconoTurno className="h-5 w-5" /> Programar turno</Link>}</section>

        <section className="mt-5 grid grid-cols-3 gap-3"><Resumen titulo="Agentes" valor={guardias.filter((g) => g.activo).length}/><Resumen titulo="En puesto" valor={activos.length} normal/><Resumen titulo="Puestos" valor={puestos.length}/></section>

        {perfil.rol === "admin" && <div className="mt-5 grid gap-4 lg:grid-cols-2">
          <details className="group rounded-2xl border border-[#27425e] bg-[#07172a]/95 open:border-[#0788ff]/50">
            <summary className="flex cursor-pointer list-none items-center justify-between gap-3 p-4 sm:p-5 [&::-webkit-details-marker]:hidden"><div><p className="text-sm font-semibold text-[#65c8ff]">Alta de personal</p><h2 className="mt-1 text-xl font-semibold">Registrar agente de seguridad</h2><p className="mt-1 text-sm text-slate-400">Uno a la vez. Crea su ficha operativa; el acceso se crea después en Usuarios y permisos.</p></div><span className="text-slate-500 transition group-open:rotate-180">⌄</span></summary>
            <div className="border-t border-[#20374e] p-4 sm:p-5"><FormularioAgente /></div>
          </details>
          <details className="group rounded-2xl border border-[#27425e] bg-[#07172a]/95 open:border-[#0788ff]/50">
            <summary className="flex cursor-pointer list-none items-center justify-between gap-3 p-4 sm:p-5 [&::-webkit-details-marker]:hidden"><div><p className="text-sm font-semibold text-[#65c8ff]">Carga masiva</p><h2 className="mt-1 text-xl font-semibold">Subir listado de personal (Excel)</h2><p className="mt-1 text-sm text-slate-400">Crea los agentes nuevos y actualiza los que ya existen, por cédula.</p></div><span className="text-slate-500 transition group-open:rotate-180">⌄</span></summary>
            <div className="border-t border-[#20374e] p-4 sm:p-5"><FormularioCargaPersonal /></div>
          </details>
        </div>}
        ...
        <section className="overflow-hidden rounded-2xl border border-[#27425e] bg-[#07172a]/95"><div className="border-b border-[#20374e] px-4 py-4"><h2 className="text-lg font-semibold">Equipo de seguridad</h2></div><div className="divide-y divide-[#20374e]">{guardias.length === 0 ? <Vacio texto="No hay personal registrado."/> : guardias.map((guardia) => {
          ...
          const puestoTurno = turno ? uno(turno.puestos) : null;
          // La plaza fija (Dotación) es lo que se muestra; el turno de hoy solo si es en otro puesto.
          const plaza = uno(guardia.plaza);
          const textoPlaza = plaza ? `${uno(plaza.empresas_cliente)?.nombre ?? ""} · ${plaza.codigo}` : guardia.es_relevo ? "Relevo · sin plaza fija" : "Sin plaza asignada";
          const puesto = puestoTurno && (!plaza || puestoTurno.codigo !== plaza.codigo) ? { codigo: textoPlaza, nombre: `hoy en ${puestoTurno.codigo}` } : { codigo: textoPlaza, nombre: "" };
          const abierto = turno ? (turno.aperturas_turno?.length ?? 0) > 0 : false;
          return <Link key={guardia.id} href={`/operacion/personal/${guardia.id}`} className="grid grid-cols-[3rem_1fr_auto_1.25rem] items-center gap-3 px-4 py-3.5 transition hover:bg-[#0b2035]/70 focus-visible:bg-[#0b2035]/70 focus-visible:outline-none"><span className="grid h-12 w-12 place-items-center rounded-full border border-[#38526b] bg-gradient-to-br from-[#244868] to-[#0a1e34] text-xs font-semibold text-[#8ddaff]">{iniciales(guardia.nombre)}</span><div className="min-w-0"><p className="truncate font-semibold">{guardia.nombre}</p><p className="truncate text-sm text-slate-400">{puesto.codigo}{puesto.nombre ? ` · ${puesto.nombre}` : ""}{!guardia.activo ? " · dado de baja" : ""}</p><p className="mt-1 text-xs text-slate-500">Credencial {guardia.credencial ?? "pendiente"}</p></div><span className={`whitespace-nowrap rounded-full px-3 py-1.5 text-xs font-medium ${abierto ? "bg-emerald-500/12 text-emerald-300" : turno ? "bg-amber-500/12 text-amber-300" : "bg-slate-500/10 text-slate-400"}`}>{abierto ? "En puesto" : turno ? "Programado" : "Disponible"}</span><IconoFlecha className="h-4 w-4 text-slate-500" /></Link>;
        })}</div></section>

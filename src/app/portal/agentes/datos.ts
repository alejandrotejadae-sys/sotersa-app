...
export type AgenteCliente = {
  ...
  puesto: { id: string; codigo: string; nombre: string } | null;
  esRelevo: boolean;
  estado: "en_puesto" | "programado" | "fuera";
};

export async function agentesDeEmpresa(empresaId: string): Promise<{ agentes: AgenteCliente[]; puestos: { id: string; codigo: string; nombre: string }[] }> {
  const administrador = crearClienteAdministrador();
  const { data: puestos } = await administrador.from("puestos").select("id,codigo,nombre").eq("empresa_cliente_id", empresaId).eq("activo", true).order("codigo");
  ...
  const [fijosR, turnosR] = await Promise.all([
    administrador.from("guardias").select("id,nombre,credencial,es_relevo,puesto_habitual_id").eq("activo", true).in("puesto_habitual_id", idsPuestos).order("nombre"),
    administrador.from("turnos").select("guardia_id,puesto_id,estado,aperturas_turno(id),guardias(id,nombre,credencial,es_relevo,activo)").in("puesto_id", idsPuestos).gte("fin_programado", desde).lte("inicio_programado", hasta).neq("estado", "ausente"),
  ]);

  const porPuesto = new Map((puestos ?? []).map((p) => [p.id, { id: p.id, codigo: p.codigo, nombre: p.nombre }]));
  ...
  for (const g of fijosR.data ?? []) {
    ...
    agentes.set(g.id, { id: g.id, nombre: g.nombre, credencial: g.credencial, puesto: porPuesto.get(g.puesto_habitual_id ?? "") ?? null, esRelevo: false, estado: turno ? (turno.abierto ? "en_puesto" : "programado") : "fuera" });
  }
  ...
  for (const t of turnosR.data ?? []) {
    ...
    agentes.set(g.id, { id: g.id, nombre: g.nombre, credencial: g.credencial, puesto: porPuesto.get(t.puesto_id) ?? null, esRelevo: true, estado: abierto ? "en_puesto" : "programado" });
  }

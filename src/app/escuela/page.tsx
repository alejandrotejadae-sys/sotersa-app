    ...
    <MarcoEscuela volver={volver}>
      <header className="mt-6 text-center">
        ...
        <p className="mt-3 text-sm leading-6 text-slate-400">
          {perfil.rol === "cliente"
            ...
            : `Hola, ${perfil.nombre.split(" ")[0]}. Tres módulos, ${TOTAL_LECCIONES} lecciones cortas y una evaluación por módulo. Puedes hacerlo por partes, desde el teléfono, entre turnos.`}
        </p>
      </header>
      ...
      {perfil.rol !== "cliente" && (
        <section className="mt-6 grid grid-cols-3 gap-3">
          ...
          <Dato titulo="Avance" valor={`${Math.round((hechas / TOTAL_LECCIONES) * 100)} %`} />
        </section>
      )}
            ...
            <Link key={m.id} href={`/escuela/${m.id}`} className={`block rounded-2xl border ${t.borde} bg-[#07172a]/90 p-4 transition hover:bg-[#08203a] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0788ff]`}>
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <h2 className="mt-1 text-lg font-semibold">{m.titulo}</h2>

...
export function FormularioRonda({ turnoId, guardiaId, puntos, completados, soloLectura }: { turnoId: string; guardiaId: string; puntos: Punto[]; completados: string[]; soloLectura: boolean }) {
  ...
  async function registrar(evento: React.FormEvent) {
    ...
    setGuardando(true);
    const { error: fallo } = await crearClienteNavegador().from("rondas").insert({ turno_id: turnoId, punto_id: punto.id, guardia_id: guardiaId, hora_captura: new Date().toISOString() });
    setGuardando(false);
  ...
  return (
    <form onSubmit={registrar} className="mt-5 rounded-2xl border border-[#27425e] bg-[#07172a]/95 p-4">
      ...
      {soloLectura ? <p className="mt-3 rounded-xl border border-[#27425e] bg-[#061426] px-4 py-3 text-sm text-slate-400">Vista de administración: el registro debe realizarlo el agente de seguridad desde su cuenta.</p> : <><label className="mt-4 block"><span className="text-sm text-slate-300">Código QR o código del punto</span><input value={codigo} onChange={(evento) => setCodigo(evento.target.value)} placeholder="Escanea o escribe el código" className="mt-2 min-h-14 w-full rounded-xl border border-[#087dd8] bg-[#020b18] px-4 text-white outline-none placeholder:text-slate-500"/></label>{error && <p role="alert" className="mt-3 rounded-xl border border-red-500/30 bg-red-500/10 px-4 py-3 text-sm text-red-200">{error}</p>}<button type="submit" disabled={guardando || !codigo.trim()} className="boton-primario mt-4 min-h-14 w-full rounded-xl font-semibold text-white disabled:opacity-50">{guardando ? "Registrando…" : "Confirmar punto"}</button></>}
    </form>
  );
}
